...
function startApp() {
  ...
  var query =
    "SELECT position, product_name, department_name, customer_price FROM products ORDER BY product_name";
  ...
  connection.query(
    ...
    function(err, res) {
      ...
      if (res[0].stock_quantity >= data.qty) {
        ...
        var newQty = {
          stock_quantity: res[0].stock_quantity - data.qty
        };
        connection.query(
          query2,
          [
        ...
        console.log(
          "There are only " + res[0].stock_quantity + " items remaing!"
        );
        return cb();
      }
    }
